...
std::vector<Polygon_2> computeBCD(const PolygonWithHoles& polygon_in,
    ...
    std::vector<VertexConstCirculator> sorted_vertices = getXSortedVertices(rotated_polygon);
...
void processEvent(const PolygonWithHoles& pwh, const VertexConstCirculator& v,
    ...
    if (less_x_2(e_prev.target(), e_prev.source()) &&
        ...
        std::list<Polygon_2>::iterator cell = std::next(open_polygons->begin(), cell_id);
...
std::vector<Point_2> getIntersections(const std::list<Segment_2>& L, const Line_2& l) {
    ...
    for (std::list<Segment_2>::const_iterator it = L.begin(); it != L.end(); ++it) {
